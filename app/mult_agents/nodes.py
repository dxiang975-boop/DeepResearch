...
2. direct_answer_node：简单问题直接调用模型回答。
3. plan_node：复杂问题拆成大纲、子问题和检索计划。
4. web_search_node：按检索计划调用 Bocha Web Search 获取网页证据。
5. local_rag_node：按检索计划查询本地 Milvus/RAG 知识库。
6. deep_dive_node：合并 web/local 证据，做评分、去重和来源索引。
7. analyze_node：基于证据池生成结论，并判断是否还需要补搜。
...
from .local_summary import collect_structured_local_records
from .state import ResearchState
from .tools import academic_search_records, bocha_web_search_records, search_knowledge_base_records
...
def colorize(text: str, color: str) -> str:
    """给日志文本加 ANSI 颜色；设置 NO_COLOR 时禁用颜色。"""
...
def emit(node: str, content: str):
    ...
    这里只打印前 400 个字符，避免长报告或大段 JSON 把日志刷屏。
    """
...
def collect_tool_calls(messages) -> tuple[list, list]:
    """从 LangChain 消息列表中收集工具调用名称和工具输出摘要。

    当前项目的核心节点多数没有真正绑定 tools，而是由节点函数自己调用
    Bocha/Milvus；这个函数主要用于调试模型是否产生了 tool call。
    """
    tools = []
    tool_outputs = []
    for msg in messages:
...
def with_memory_context(state: ResearchState, user_prompt: str) -> str:
    """把记忆上下文追加到当前 prompt 后面。

    WorkflowService 会提前把相关记忆写入 state["memory_context"]。
    节点调用模型前通过这个函数注入记忆，让模型知道历史偏好或上下文。
    """
...
def log_inputs(node: str, agent_name: str, payload: dict):
    """打印节点输入摘要，便于调试每个 Agent 收到了哪些关键信息。"""
    preview = {
...
def detect_intent(query: str) -> str:
    ...
    这个函数不会直接决定最终路由，而是给 intent_node 的 LLM 判断提供
    一个 rule_route 参考。它的策略很简单：
    - 命中“调研、趋势、新闻、证据、来源”等关键词，倾向 multiagent。
    - 否则倾向 direct。
    """
...
def bind_agent(node_func, agent, agent_name: str):
    """把节点函数和具体 Agent 实例绑定成 LangGraph 可调用函数。

    graph.py 注册节点时，LangGraph 只会传入 state。节点函数实际还需要
    agent 和 agent_name，所以这里用 partial 提前绑定。
    """
    return partial(node_func, agent=agent, agent_name=agent_name)


def _last_content(result) -> str:
    """从 LangChain agent.invoke 的返回结果中取最后一条模型消息文本。"""
...
def _extract_json_block(text: str) -> str:
    ...
    很多节点要求模型“只输出 JSON”，但模型有时会包一层 ```json 代码块，
    或在 JSON 前后加解释。这里尽量截取第一个 { 到最后一个 }。
    """
    cleaned = text.strip()
...
def _load_json(text: str, fallback: dict) -> dict:
    """把模型输出解析为 dict；解析失败时返回 fallback。

    这是项目里的重要容错点：如果 LLM 没按 JSON 格式输出，工作流不会直接崩，
    而是使用本地规则生成的 fallback 结果继续向下走。
    """
    try:
...
def _invoke_json_agent(state: ResearchState, prompt: str, agent, agent_name: str, node: str, fallback: dict) -> tuple[dict, str, list]:
    """调用“期望输出 JSON”的 Agent，并返回解析后的结构化结果。

    返回三项：
    - payload：解析后的 JSON dict，失败则 fallback。
    - content：模型原始文本输出。
    - messages：本轮 HumanMessage 和 AIMessage，用于追加到 state["messages"]。

    设计取舍：
    这里没有把历史 state["messages"] 全量传给模型，而是只传当前节点需要的
    prompt 和必要 state 字段，目的是控制 token 成本并减少前面 JSON 输出对
    后续 Writer 的格式污染。
    """
    human = HumanMessage(content=with_memory_context(state, prompt))
    # 优化点：不要把 state["messages"] 全部传入，避免 token 累积。
    # 每个节点只需要当前节点的指令，以及 prompt 中显式注入的 state 数据。
...
def _derive_direct_search_queries(query: str) -> list[str]:
    ...
    Planner 生成的 search_queries 可能太发散，所以这里先强制加入一组
    与用户原问题直接相关的查询，降低检索跑偏概率。
    """
...
def _derive_search_plan(outline: list[dict], sub_questions: list[str], _research_questions: list[str], query: str) -> list[dict]:
    """把 Planner 输出的大纲/子问题转成真正要执行的 search_plan。

    search_plan 的每一项一般包含：
    - section_id：属于哪个报告章节或缺口。
    - query：实际检索词。
...
def _build_queries(state: ResearchState, source_preference: str) -> list[dict]:
    ...
    第一轮使用 state["search_plan"]。
    reflect 补搜轮使用 state["supplementary_queries"]。
    source_preference 用来区分 web_search_node 和 local_rag_node 各自应该执行
    哪些查询。
    """
    queries: list[dict] = []
...
def _filter_web_records(query: str, records: list[dict]) -> tuple[list[dict], dict]:
    ...
    当前节点后面还会让 LLM 做一次结构化筛选；这里先用标题、摘要、域名和
    关键词相关性进行预过滤，减少无关内容进入 prompt。
    """
    kept = []
...
def _format_raw_records(records: list[dict], source_type: str) -> str:
    """把原始检索结果压缩成 JSON Lines，注入给 LLM 做证据整理。"""
    if not records:
        return "[]"
    lines = []
    for record in records[:40]:
        locator = record.get("url") or record.get("doc_id") or ""
        snippet_limit = 1800 if str(record.get("source_category", "")).startswith("structured_") else 500
        lines.append(
            json.dumps(
                {
                    ...
                    "snippet": str(record.get("snippet", ""))[:snippet_limit],
...
def _assign_source_ids(records: list[dict], prefix: str) -> list[dict]:
    """给每条原始检索记录分配稳定 source_id。

    source_id 是后续证据审计、分析结论、最终引用的唯一凭证。
    """
    assigned: list[dict] = []
...
def _prune_evidence_to_allowed_sources(evidence: list[dict], allowed_source_ids: set[str]) -> list[dict]:
    """只保留来源 ID 确实来自原始检索结果的证据，过滤 LLM 编造来源。"""
    kept: list[dict] = []
...
def _summarize_records(records: list[dict]) -> list[dict]:
    """把原始记录压缩成 trace 摘要，用于前端/日志展示检索过程。"""
    summary: list[dict] = []
...
def _finalize_query_traces(query_traces: list[dict], kept_ids: set[str], rejected_ids: list[str], reject_reason: str) -> list[dict]:
    """给每个查询 trace 补充 kept/rejected 信息，便于观察证据筛选过程。"""
...
def _fallback_web_evidence(records: list[dict]) -> dict:
    """WebScout 的兜底结果：LLM 解析失败时直接把原始网页记录转成 evidence。"""
    evidence = []
    for record in records:
        evidence.append(
            {
...
def _fallback_local_evidence(records: list[dict]) -> dict:
    """LocalRAGScout 的兜底结果：LLM 解析失败时直接把本地记录转成 evidence。"""
    evidence = []
    for record in records:
        evidence.append(
            {
...
def _fallback_audit(state: ResearchState) -> dict:
    """EvidenceJudge 的兜底审计逻辑。

    如果证据裁判 Agent 没返回合法 JSON，就用规则把 web/local evidence 合并成
    evidence_pool，并生成 source_index，保证后续 Analyst/Writer 仍能运行。
    """
    evidence_pool = []
    source_index = []
    audit_flags = []
...
def _fallback_analysis(state: ResearchState) -> dict:
    ...
    根据 evidence_pool 生成最基本的 findings/claim_map，并判断哪些子问题缺证据。
    """
...
def _build_source_lookup(state: ResearchState) -> dict[str, dict]:
    """建立 source_id -> source 信息的查询表。

    Writer 引用校验、参考资料列表渲染都依赖这个 lookup。
    """
...
def _extract_citation_ids(content: str) -> list[str]:
    """从正文中提取所有引用ID [XXX]"""
    matches: list[str] = []
    for bracket_content in re.findall(r"\[([^\]]+)\]", content):
        matches.extend(re.findall(r"\b[A-Z]+[A-Z0-9_]*-\d+\b", bracket_content))
...
def _validate_and_fix_citations(content: str, valid_source_ids: set[str]) -> tuple[str, list[str]]:
    ...
    def replace_citation_group(match):
        ...
        citation_ids = re.findall(r"\b[A-Z]+[A-Z0-9_]*-\d+\b", bracket_content)
        valid_ids = [citation_id for citation_id in citation_ids if citation_id in valid_source_ids]
        if not valid_ids:
            return ""
        return "".join(f"[{citation_id}]" for citation_id in dict.fromkeys(valid_ids))

    fixed_content = re.sub(r"\[([^\]]+)\]", replace_citation_group, content)
    # 提取修正后实际使用的合法引用
...
def _render_reference_list(state: ResearchState) -> str:
    """根据 source_index/evidence_pool 渲染“参考资料”区块。"""
    lines = ["## 参考资料"]
...
def _render_execution_appendix(state: ResearchState) -> str:
    """渲染执行附录，展示检索 query、保留/拒绝的来源和统计信息。

    这部分不是核心答案，但对调试和简历演示很有价值：它能说明系统不是黑盒，
    而是有可观测的检索过程和证据筛选过程。
    """
    lines = ["## 规划与检索明细", "", "### 执行概览"]
...
def intent_node(state: ResearchState, agent, agent_name: str) -> ResearchState:
    ...
    处理：
    1. 先用 detect_intent 做规则初判，得到 rule_route。
    2. 再让 IntentRouter Agent 输出 JSON：{"route": "direct|multiagent"}。
    3. 如果 LLM 输出不合法，就回退到 rule_route。

    输出更新：
    - intent：direct 或 multiagent，供 graph.py 的 route_after_intent 分流。
    - draft：保存模型原始输出，便于调试。
    - messages：保存本节点人类消息和模型消息。
    """
...
def direct_answer_node(state: ResearchState, agent, agent_name: str) -> ResearchState:
    ...
    当 intent_node 判断问题是闲聊/简单问答时，Graph 会走到这里。
    这个节点不会检索网页，也不会做证据审计，而是直接调用模型生成回答。

    输出更新：
    - final：最终答案。
    - draft/analysis_summary：保存同一份直接回答。
    - needs_more_research=False：明确不进入补搜。
    """
...
def plan_node(state: ResearchState, agent, agent_name: str) -> ResearchState:
    ...
    处理：
    1. 调用 Planner Agent，让它输出 objective、sub_questions、outline、budget。
    2. 使用 _derive_search_plan 把大纲中的搜索建议转成可执行 search_plan。
    3. 如果 LLM JSON 解析失败，就使用 _default_plan。

    输出更新：
    - plan：研究目标摘要。
    - outline：报告大纲。
    - sub_questions/research_questions：拆解后的问题。
    - search_plan：web/local 检索节点要执行的查询计划。
    - budget：轮数、来源数、token 等预算信息。
    - iteration=0：初始化研究迭代轮次。
    """
...
def web_search_node(state: ResearchState, agent, agent_name: str) -> ResearchState:
    ...
    输入：
    - state["search_plan"] 或补搜轮的 state["supplementary_queries"]。

    处理：
    1. _build_queries 选出本轮需要 web 执行的 query。
    2. 调用 bocha_web_search_records 获取网页标题、链接、摘要。
    3. 给每条结果分配 WEB... source_id，并记录 web_search_trace。
    4. 去重、过滤空记录，然后让 WebScout Agent 整理结构化 evidence。
    5. 校验 evidence 的 source_id 必须来自原始检索结果，防止模型编造来源。

    输出更新：
    - web_evidence：累积网页证据。
    - web_retrieval_stats：网页检索统计。
    - web_search_trace：每个 query 的原始/保留/拒绝记录。
    """
    ...
    for query_index, item in enumerate(queries, 1):
        ...
        if _detect_task_mode(state.get("query", "")) == "science":
            records.extend(academic_search_records(query_text, count=5))
    ...
    academic_records = {
        ...
        if item.get("source_id") and str(item.get("source_category", "")) in {"academic", "leaderboard", "code"}
    }
    existing_evidence_ids = {str(item.get("source_id")) for item in evidence if item.get("source_id")}
    for source_id, record in academic_records.items():
        if source_id in existing_evidence_ids:
            continue
        evidence.append(
            {
                "source_id": source_id,
                ...
                "source_type": "web",
                "source_category": record.get("source_category", ""),
                "reliability_hint": record.get("reliability_hint", "academic"),
                "supports_questions": state.get("sub_questions", []),
                "notes": "学术搜索补充来源，来自 arXiv/Semantic Scholar 或学术/榜单/代码站点。",
            }
        )
...
def local_rag_node(state: ResearchState, agent, agent_name: str) -> ResearchState:
    ...
    作用和 web_search_node 类似，但数据来源不是互联网，而是本地向量知识库。

    处理：
    1. _build_queries 选出本轮需要 local 执行的 query。
    2. search_knowledge_base_records 查询 Milvus/RAG。
    3. 给每条结果分配 LOC... source_id。
    4. 让 LocalRAGScout Agent 把原始 chunk 整理成 local_evidence。
    5. 校验 source_id，防止模型编造本地文档来源。

    输出更新：
    - local_evidence：累积本地证据。
    - local_retrieval_stats：本地检索统计。
    - local_rag_trace：本地查询 trace。
    """
    ...
    structured_records = collect_structured_local_records(state.get("query", "")) if iteration == 0 else []
    structured_records = _assign_source_ids(structured_records, f"{prefix}_STRUCT")
    for record in structured_records:
        record["section_id"] = "structured_local_summary"
        record["search_query"] = state.get("query", "")
    raw_records.extend(structured_records)
    if structured_records:
        query_traces.append(
            {
                "iteration": iteration,
                "plan_step": "structured_summary",
                "query": state.get("query", ""),
                "section_id": "structured_local_summary",
                "reason": "全量读取 CSV/log/PDF 生成结构化摘要，避免只依赖 Milvus top-k chunk",
                "source_preference": "local",
                "raw_count": len(structured_records),
                "raw_records": _summarize_records(structured_records),
            }
        )
    ...
    structured_by_id = {
        str(item.get("source_id")): item
        for item in raw_records
        if str(item.get("source_category", "")).startswith("structured_") and item.get("source_id")
    }
    existing_evidence_ids = {str(item.get("source_id")) for item in evidence if item.get("source_id")}
    for source_id, record in structured_by_id.items():
        if source_id in existing_evidence_ids:
            continue
        evidence.append(
            {
                ...
                "source_type": "local",
                "source_category": record.get("source_category", ""),
                "reliability_hint": record.get("reliability_hint", "internal"),
                "doc_type": record.get("doc_type", ""),
                "supports_questions": state.get("sub_questions", []),
                "notes": "结构化本地摘要，来自原始 CSV/log/PDF 的全量读取结果。",
            }
        )
...
def deep_dive_node(state: ResearchState, agent, agent_name: str) -> ResearchState:
    ...
    处理：
    1. 调用 EvidenceJudge Agent，对 web/local 证据做评分、去重、冲突检测。
    2. 只接受原始 evidence 中真实存在的 source_id。
    3. 如果 LLM 漏掉部分真实证据，用本地 _score_evidence 规则补入。
    4. 构造 source_index，供 Writer 最终引用和参考资料渲染。

    输出更新：
    - evidence_pool：统一证据池。
    - audit_flags：低置信度、冲突、缺证据等审计标记。
    - source_index：可引用来源索引。
    """
        ...
        evidence_pool.append(
            {
                ...
                "snippet": record.get("snippet", ""),
                "source_category": record.get("source_category", ""),
                "reliability_hint": record.get("reliability_hint", ""),
                "doc_type": record.get("doc_type", ""),
...
def analyze_node(state: ResearchState, agent, agent_name: str) -> ResearchState:
    ...
    输入：
    - state["evidence_pool"]：EvidenceJudge 输出的统一证据池。
    - state["audit_flags"]：证据风险或缺口提示。
    - state["sub_questions"]：Planner 拆出来的子问题。

    处理：
    1. 调用 Analyst Agent，把证据池转成 findings 和 claim_map。
    2. 判断证据是否足够回答所有子问题。
    3. 如果不足，设置 needs_more_research=True，并输出 missing_gaps。

    输出更新：
    - analysis：分析摘要。
    - findings：最终报告要展开的核心结论。
    - claim_map：结论和来源 ID 的映射关系。
    - needs_more_research/missing_gaps：供 graph.py 判断是否进入 reflect。
    """
...
def reflect_node(state: ResearchState, agent, agent_name: str) -> ResearchState:
    ...
    只有 analyze_node 判断证据不足时才会进入这里。

    输入：
    - state["missing_gaps"]：当前还缺哪些信息。
    - state["search_plan"]：已经执行过的初始搜索计划。
    - state["supplementary_queries"]：此前补搜过的查询。

    处理：
    - 复用 Planner Agent，根据缺口生成新的 supplementary_queries。
    - iteration + 1，表示研究进入下一轮补搜。

    输出更新：
    - iteration：当前补搜轮次。
    - supplementary_queries：下一轮 web_search/local_rag 要执行的查询。
    """
...
def write_node(state: ResearchState, agent, agent_name: str) -> ResearchState:
    ...
    输入：
    - findings：分析节点产出的结论。
    - source_index：证据裁判节点产出的合法来源索引。
    - audit_flags：风险、冲突或缺证据提示。

    处理：
    1. 把合法 source_id 列表显式传给 Writer Agent。
    2. 要求模型输出 Markdown 正文，而不是 JSON。
    3. 清理模型可能包裹的代码块标记。
    4. _validate_and_fix_citations 移除非法引用 ID，抑制幻觉引用。
    5. _ensure_reference_section 自动追加参考资料和执行附录。

    输出更新：
    - draft：最终 Markdown 报告。
    - final：返回给前端/接口的最终答案。
    """
...
# ---------------------------------------------------------------------------
# 科研助手 / 行业研究助手增强规则
# 这些函数保持原有函数名，Python 会在模块加载后使用最后一次定义的版本。
# 这样可以增强意图识别、搜索词生成和证据评分，同时不改 LangGraph 的节点和边。

def _detect_task_mode(query: str) -> str:
    text = query.lower()
    if "当前任务模式：科研助手" in query or "科研助手" in query:
        return "science"
    if "当前任务模式：行业研究助手" in query or "行业研究助手" in query:
        return "industry"
    science_keywords = [
        "论文",
        "文献",
...
def detect_intent(query: str) -> str:
    """规则增强版意图初判：科研/行业研究默认进入多 Agent 深度链路。"""
    normalized_query = query.strip()
    if not normalized_query:
        return "direct"
    if _detect_task_mode(normalized_query) in {"science", "industry"}:
        return "multiagent"
    if re.search(r"20\d{2}年?", normalized_query) and any(word in normalized_query for word in ["趋势", "新闻", "调研", "调查", "盘点"]):
        return "multiagent"
    keywords = [
    ...
    return "multiagent" if any(word in normalized_query.lower() for word in keywords) else "direct"


def _derive_direct_search_queries(query: str) -> list[str]:
    """根据任务模式生成更贴近原问题的直接检索词。"""
    base_query = query.strip()
    if not base_query:
        return []
    mode = _detect_task_mode(base_query)
    entity = _guess_primary_entity(base_query)
    target = entity or base_query
    candidates = [base_query]

    if mode == "science":
        candidates.extend(
            [
                f"{target} survey recent papers",
                f"{target} state of the art benchmark",
                f"{target} papers with code",
                f"{target} arxiv",
                f"{target} experiment results ablation baseline",
                f"{target} dataset metrics comparison",
            ]
        )
    elif mode == "industry":
        candidates.extend(
            [
                f"{target} 行业研究 市场规模 趋势",
                f"{target} 竞品分析 商业模式",
                f"{target} 政策 产业链 风险",
                f"{target} market size trend report",
                f"{target} competitors business model funding",
                f"{target} annual report white paper",
            ]
        )
    elif entity:
        candidates.extend(
...
def _score_evidence(record: dict) -> tuple[float, str]:
    """增强版证据评分：兼顾科研论文、榜单、代码、官方报告和本地实验资料。"""
    source_type = record.get("source_type")
    if source_type == "local":
        doc_type = str(record.get("doc_type") or record.get("metadata", {}).get("doc_type") or "").lower()
        if doc_type in {"experiment_table", "experiment_log"}:
            return 0.95, "本地实验结果或实验日志，适合作为科研助手分析的核心证据"
        if doc_type in {"paper_or_document", "pdf", "docx"}:
            return 0.93, "本地论文或文档证据，默认高可信"
        return 0.92, "本地知识库证据，默认高可信"

    category = str(record.get("source_category") or "").lower()
    hint = str(record.get("reliability_hint") or "").lower()
    domain = str(record.get("domain", "")).lower()
    value = f"{category} {hint} {domain} {record.get('url', '')}".lower()

    if category == "leaderboard" or "paperswithcode.com" in value:
        return 0.9, "公开榜单或 benchmark 来源，适合 SOTA 对比"
    if category == "academic" or any(item in value for item in ["arxiv.org", "openreview.net", "aclanthology.org", "ieee.org", "acm.org"]):
        return 0.88, "学术论文或会议/期刊来源"
    if category == "code" or any(item in value for item in ["github.com", "huggingface.co"]):
        return 0.82, "官方代码、模型或复现资源，可辅助验证方法细节"
    if category in {"official", "industry_report"} or _is_official_domain(domain):
        return 0.86, "官方、政府、机构报告或权威行业资料"
    if category == "media" or any(word in domain for word in ["news", "finance", "reuters", "bloomberg", "people", "xinhuanet"]):
        return 0.72, "主流媒体来源，适合补充事实但需要交叉验证"
    if category == "community":
        return 0.5, "社区或博客来源，可信度较低，仅适合作为线索"
    if domain:
        return 0.58, "普通互联网来源，需要交叉验证"
    return 0.45, "来源信息不完整"
