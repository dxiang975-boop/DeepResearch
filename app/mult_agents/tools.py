...
def _classify_web_source(domain: str, url: str = "", title: str = "") -> tuple[str, str]:
    """给网页来源做轻量分类，供证据裁判和最终报告使用。"""
    value = f"{domain} {url} {title}".lower()
    academic_domains = [
        "arxiv.org",
        "openreview.net",
        "aclanthology.org",
    ...
    if any(item in value for item in ["paperswithcode.com", "leaderboard", "benchmark"]):
        return "leaderboard", "leaderboard"
    if any(item in value for item in academic_domains) or ".edu" in value:
        return "academic", "academic"
    if any(item in value for item in ["github.com", "gitlab.com", "huggingface.co"]):
        return "code", "official"
    if any(item in value for item in [".gov", "gov.cn", "official", "annual report", "white paper"]):
        return "official", "official"
    if any(item in value for item in ["mckinsey", "gartner", "forrester", "idc.com", "cbinsights", "pitchbook", "statista", "report"]):
        return "industry_report", "industry_report"
    if any(item in value for item in ["reuters", "bloomberg", "wsj", "ft.com", "caixin", "xinhuanet", "people.com.cn", "36kr", "finance"]):
        return "media", "media"
    if any(item in value for item in ["reddit", "zhihu", "medium.com", "blog", "forum"]):
        return "community", "community"
    return "unknown", "unknown"


def _request_json(url: str, timeout: int = 20) -> dict:
    request = urllib.request.Request(url, headers={"User-Agent": "DeepResearch/1.0"})
    with urllib.request.urlopen(request, timeout=timeout) as response:
        return json.loads(response.read().decode("utf-8"))


def _academic_arxiv_records(query: str, count: int) -> list[dict]:
    ...
    try:
        ...
        logger.warning("[academic_search] arXiv search failed | query=%s | error=%s", query, exc)
        return []

    namespace = {"atom": "http://www.w3.org/2005/Atom"}
    records: list[dict] = []
    for entry in root.findall("atom:entry", namespace)[:count]:
        ...
        records.append(
            {
                "title": title,
                "url": link,
                "snippet": f"arXiv paper. Published: {published}. Abstract: {summary}",
                "domain": "arxiv.org",
                "source_type": "web",
                "source_category": "academic",
                "reliability_hint": "academic",
...
def _academic_semantic_scholar_records(query: str, count: int) -> list[dict]:
    params = urllib.parse.urlencode(
        {
            "query": query,
            "limit": count,
            "fields": "title,abstract,year,url,venue,citationCount,authors",
        }
    )
    ...
    try:
        ...
        logger.warning("[academic_search] Semantic Scholar search failed | query=%s | error=%s", query, exc)
        return []

    records: list[dict] = []
    for paper in data.get("data", [])[:count]:
        if not isinstance(paper, dict):
            continue
        title = str(paper.get("title") or "").strip()
        if not title:
            continue
        authors = ", ".join(author.get("name", "") for author in paper.get("authors", [])[:4] if isinstance(author, dict))
        ...
        records.append(
            {
                ...
                "snippet": f"Semantic Scholar paper. Year: {year}. Venue: {venue}. Citations: {citations}. Authors: {authors}. Abstract: {abstract}",
...
def academic_search_records(query: str, count: int = 6) -> list[dict]:
    """面向科研助手的免费学术搜索补充源：arXiv + Semantic Scholar。"""
    normalized = query.strip()
    if not normalized:
        return []
    records: list[dict] = []
    records.extend(_academic_arxiv_records(normalized, max(1, count // 2)))
    records.extend(_academic_semantic_scholar_records(normalized, max(1, count - len(records))))
    deduped: list[dict] = []
    seen: set[tuple[str, str]] = set()
    for record in records:
        key = (str(record.get("title", "")).lower(), str(record.get("url", "")).lower())
        if key in seen:
            continue
        seen.add(key)
        deduped.append(record)
    logger.info("[academic_search] query=%s | records=%s", query, len(deduped))
...
def bocha_web_search_records(query: str, count: int = 8) -> list[dict]:
    ...
    for idx, page in enumerate(pages[:count], 1):
        ...
        source_category, reliability_hint = _classify_web_source(domain, url, str(title))
